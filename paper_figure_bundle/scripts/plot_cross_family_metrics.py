...
def family_x_offset(fam: str) -> float:
    """Small deterministic jitter so equal-size families remain visible."""
    return {
        "Qwen2.5": -0.08,
        "Llama 3.x": 0.00,
        "Gemma3": 0.08,
...
def plot_family_condition_legend(ax: plt.Axes) -> None:
    ...
    family_handles = [
        Line2D(
            ...
            marker=MARKERS[fam],
            color="none",
            markerfacecolor=COLORS[fam],
            markeredgecolor=COLORS[fam],
            markersize=6,
            label=fam,
        )
        for fam in FAMILIES
    ...
    handles = family_handles + condition_handles
    ax.legend(
        handles=handles,
        loc="upper center",
...
def plot_metric_vs_size_markers(
    ax: plt.Axes,
    rows: list[dict[str, str]],
    *,
    metric: str,
    ...
    """Final metric vs model size, marker-only with paired condition gaps."""
    for fam in FAMILIES:
        biased_grouped = grouped_final_metric(rows, fam=fam, biased=True, metric=metric)
        unbiased_grouped = grouped_final_metric(rows, fam=fam, biased=False, metric=metric)
        sizes = sorted(set(biased_grouped) | set(unbiased_grouped))
        x_offset = family_x_offset(fam)
        for size in sizes:
            x = size + x_offset
            summaries: dict[bool, tuple[float, float, int]] = {}
            for biased, grouped in ((True, biased_grouped), (False, unbiased_grouped)):
                vals = grouped.get(size, [])
                m, s, n = values_mean_std(vals)
                if n:
                    summaries[biased] = (m, s, n)
            if True in summaries and False in summaries:
                ax.plot(
                    [x, x],
                    [summaries[False][0], summaries[True][0]],
                    color=COLORS[fam],
                    alpha=0.20,
                    lw=1.2,
                    zorder=1,
                )
            for biased, fill, alpha in ((False, False, 0.95), (True, True, 1.0)):
                if biased not in summaries:
                    continue
                m, s, _n = summaries[biased]
                ax.errorbar(
                    [x],
                    [m],
                    yerr=[s],
    ...
    if ylim:
        ax.set_ylim(*ylim)
    ax.set_xlim(0, 14.8)
    ax.set_xticks([0.5, 1, 1.5, 3, 4, 7, 8, 12, 14])
    ax.set_xticklabels(
        ["0.5", "1", "1.5", "3", "4", "7", "8", "12", "14"],
        rotation=30,
        ha="right",
    )
    style_axes(ax)
    ...
    if add_legend:
        plot_family_condition_legend(ax)
...
def main() -> None:
    ...
    plot_accuracy_a_rate_side_by_side(rows, out_dir)
    plot_metric_vs_size_overlay(
        rows,
