...
FULL FINE-TUNING VARIANT — updates ALL model weights directly (no LoRA, no
4-bit quantization). Mirrors the recipe validated in final_full_finetune.ipynb:
  * FastLanguageModel.from_pretrained(..., full_finetuning=True,
    load_in_4bit=False, load_in_8bit=False)
  * NO FastLanguageModel.get_peft_model(...) call
  * torch._dynamo.config.suppress_errors = True (keeps Unsloth's compiler ON
    so hidden states still flow to GRPO's memory-efficient logps path)
  * gradient_checkpointing=True in GRPOConfig (needed for full FT to fit in VRAM)

Storage policy: full-FT checkpoints are full model weights (~3 GB for a 1.5B
model in bf16), so NO intermediate checkpoints are written during training
(save_steps is effectively disabled). Only FINAL models are saved, and only
where required:
  * condition 0 ("the bare hack") ALWAYS saves its final model — it is the
    hacked policy that conditions 3a/3b/recovery resume from via --hacked-ckpt.
  * all other conditions skip the weight dump by default (their results live
    in the final_eval JSONs); pass --save-final-model always to force saving.
Control where final models go with --ckpt-dir (default: <output_root>/checkpoints).
...
Six conditions:
  ...
  3a  Adaptive non-blind, from the condition-0 full-FT FINAL checkpoint
  3b  Adaptive blind, from the condition-0 full-FT FINAL checkpoint
...
CLI:
  # 1. Produce the hacked full-FT policy (final model auto-saved):
  python train_time_full_ft.py --condition 0 \
      --base-model Qwen/Qwen2.5-1.5B-Instruct --ckpt-dir checkpoints
  # 2. Intervention conditions (no weights saved by default):
  python train_time_full_ft.py --condition 2b --proposer-provider anthropic
  # 3. Resume conditions from the saved full-weight checkpoint:
...
# --- FIX (full fine-tuning + GRPO) ---------------------------------------
# Do NOT set UNSLOTH_COMPILE_DISABLE or torch._dynamo.config.disable here.
# Disabling Unsloth's compiler stops the model from returning HIDDEN STATES,
# but GRPO's memory-efficient logps path still expects them -> you get
# "mat1 and mat2 shapes cannot be multiplied (Nx151936 and 1536x151936)".
#
# Instead keep the compiler ON and let Dynamo fall back to eager on the one
# node it cannot trace (the lm_head matmul inside the functorch grad
# transform). This fixes the original "Dynamo failed to run FX node ...
# matmul" crash without breaking the hidden-states data path.
...
# ---- Final-model saving policy (full-FT storage control) ---------------------
# Set from CLI in main(). With full fine-tuning every checkpoint is the complete
# model (~2 bytes/param), so we save weights only where required:
#   "auto"   -> save ONLY for condition 0 (the hacked policy that 3a/3b/recovery
#               need as their --hacked-ckpt input). All other conditions are
#               fully characterized by their final_eval JSONs.
#   "always" -> save the final model for every condition.
#   "never"  -> never save weights (condition 0 included — only do this if you
#               already have a hacked checkpoint on disk).
SAVE_FINAL_MODEL_POLICY = "auto"
CKPT_DIR: Optional[Path] = None  # default resolved in maybe_save_final_model
...
def run_stage(
    *,
    ...
    args = GRPOConfig(
        ...
        # Full-FT checkpoints are full model weights (multi-GB each), so never
        # save intermediate checkpoints during training. Final models are
        # persisted (only where required) via maybe_save_final_model().
        save_steps=10**9,
        save_strategy="no",
        max_grad_norm=GRPO_GRAD_NORM,
        beta=beta,
        gradient_checkpointing=True,   # needed for full FT to fit in VRAM
        report_to="none",
...
def load_base_model(model_name: Optional[str] = None, cache_dir: Optional[str] = None):
    """Load the base model for FULL fine-tuning (no LoRA, no quantization).

    Matches the recipe validated in final_full_finetune.ipynb:
      * Only ONE of load_in_4bit / load_in_8bit / full_finetuning may be True.
      * full_finetuning=True makes every parameter trainable as-is — there is
        deliberately NO FastLanguageModel.get_peft_model(...) call here.
      * Gradient checkpointing is enabled via GRPOConfig in run_stage()
        (gradient_checkpointing=True), not via get_peft_model.
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        ...
        max_seq_length=1024,
        load_in_4bit=False,        # must be False for full FT (no quantized weights)
        load_in_8bit=False,
        full_finetuning=True,      # train every parameter directly
        cache_dir=cache_dir,
    )
...
def load_hacked_checkpoint(ckpt_path: str, cache_dir: Optional[str] = None):
    """Load the hacked FULL-WEIGHT checkpoint for conditions 3a/3b/recovery.

    Unlike the LoRA variant, the checkpoint here is a complete model directory
    (produced by maybe_save_final_model / model.save_pretrained), not an
    adapter — so we load it exactly like a base model, with full_finetuning=True
    so continued GRPO updates all weights.
    """
    model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    # Ensure it's trainable for continued GRPO
...
def maybe_save_final_model(
    *,
    ...
    """Persist the FINAL full-weight model, but only where required.

    Honors SAVE_FINAL_MODEL_POLICY / CKPT_DIR (set from CLI in main()):
      * "auto":   save only condition 0 — its final model IS the hacked policy
                  that conditions 3a/3b/recovery load via --hacked-ckpt.
      * "always": save for every condition.
      * "never":  skip entirely.

    Condition 0's checkpoint is named `stage2_reasoning_first_FINAL` so the
    default --hacked-ckpt path (<ckpt-dir>/stage2_reasoning_first_FINAL) works
    without extra flags. Other conditions save as `condition_<tag>_FINAL`.
    Returns the save path, or None if skipped.
    """
    policy = SAVE_FINAL_MODEL_POLICY
    required = (condition == "0")
    if policy == "never" or (policy == "auto" and not required):
        logger.info(
            "Skipping final model save for condition %s (policy=%s; full-FT "
            "weights are multi-GB and not required downstream).",
            condition, policy,
        )
        return None

    ckpt_root = CKPT_DIR if CKPT_DIR is not None else (output_root / "checkpoints")
    tag = (
        "stage2_reasoning_first_FINAL" if condition == "0"
        else f"condition_{condition.replace('-', '_')}_FINAL"
    )
    save_path = ckpt_root / tag
    save_path.mkdir(parents=True, exist_ok=True)

    console_banner(f"Saving FINAL full-weight model for condition {condition} -> {save_path}")
    ...
    logger.info("Final model saved: %s (%.2f GB)", save_path, size_gb)
    return save_path
...
def write_final_evals(
    *,
    ...
    # Every condition runner ends with write_final_evals, so this is the single
    # choke point for persisting final weights (only where required — see
    # maybe_save_final_model / SAVE_FINAL_MODEL_POLICY).
    maybe_save_final_model(
        model=model, tokenizer=tokenizer,
        output_root=output_root, condition=condition,
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output-root", default=None,
                   help="Output directory. Defaults to outputs/train_time_full_ft/<condition>.")
    p.add_argument(
        "--ckpt-dir",
        default=None,
        help=(
            "Where to save FINAL full-weight model checkpoints. Defaults to "
            "<output-root>/checkpoints. Condition 0 saves as "
            "<ckpt-dir>/stage2_reasoning_first_FINAL so it can be fed straight "
            "back in via --hacked-ckpt for conditions 3a/3b/recovery."
        ),
    )
    p.add_argument(
        "--save-final-model",
        ...
        default="auto",
        help=(
            "Storage policy for final full-weight models (intermediate "
            "checkpoints are never saved). 'auto' (default): save only for "
            "condition 0, whose final model is required downstream as the "
            "hacked checkpoint. 'always': save every condition's final model. "
            "'never': save nothing."
        ),
    )
    p.add_argument("--hacked-ckpt",
                   ...
                   help=(
                       "FULL-WEIGHT model checkpoint directory for conditions "
                       "3a/3b/recovery (produced by a prior --condition 0 run "
                       "of this script). Note: this is a complete saved model, "
                       "not a LoRA adapter."
                   ))
    ...
    p.add_argument(
        ...
        help=(
            ...
            "(everything except 3a/3b/recovery, which resume from --hacked-ckpt). "
            "Must be a FULL-PRECISION instruct model (full_finetuning=True is "
            "incompatible with pre-quantized `-bnb-4bit` repos), e.g. "
            "`Qwen/Qwen2.5-0.5B-Instruct`, `Qwen/Qwen2.5-3B-Instruct`, "
            "`meta-llama/Llama-3.2-1B-Instruct`, or "
            "`meta-llama/Llama-3.1-8B-Instruct`. Defaults to "
...
def main() -> None:
    ...
    output_root = Path(
        args.output_root or f"outputs/train_time_full_ft/{args.condition}"
    )
    output_root.mkdir(parents=True, exist_ok=True)

    # Final-model storage policy (full FT => multi-GB checkpoints; see
    # maybe_save_final_model). Threaded via module globals like DEFAULT_BASE_MODEL.
    global SAVE_FINAL_MODEL_POLICY, CKPT_DIR
    SAVE_FINAL_MODEL_POLICY = args.save_final_model
    ...
    logger.info("Mode: FULL fine-tuning (all weights, no LoRA, no 4-bit)")
    logger.info("Final-model save policy: %s | ckpt dir: %s",
                SAVE_FINAL_MODEL_POLICY,
                CKPT_DIR if CKPT_DIR is not None else output_root / "checkpoints")
    logger.info("=" * 70)
    console_banner(
        f"Run start | condition={args.condition} | base_model={DEFAULT_BASE_MODEL} "
        f"| FULL-FT | save_final_model={SAVE_FINAL_MODEL_POLICY} "
